...
// Keep hand-authored entries (varbit/varp quest checkpoints, captured
// targets) that this scraper didn't generate — only OUR keys refresh.
if (fs.existsSync(ANNOTATIONS_OUT)) {
  const existing = JSON.parse(fs.readFileSync(ANNOTATIONS_OUT, 'utf8')).annotations || {};
  let kept = 0;
  ...
  if (kept) console.log(`Kept ${kept} hand-authored annotation(s) not regenerated by the scraper.`);
}
